...
module Make (Eq : EQUAL) = struct
  ...
  (*@ function enum_elements (e : enum) : Eq.elt list =
        match e with
        | [] -> []
        | (x, r) :: e -> x :: (elements r @ enum_elements e) *)

  (** Exercise: complete the specification of the [mk_zipper] function *)
  ...
  (** Exercise: complete the specification of the [eq_enum] function *)
